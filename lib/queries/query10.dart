...
class Query10 extends StatefulWidget {
  Query10({Key? key, required this.conn})
      : super(key: key);
  ...
  @override
  State<Query10> createState() => _Query10State();
}

class _Query10State extends State<Query10> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SafeArea(
        child: FutureBuilder(
          future: widget.conn
              .execute("SELECT * FROM lease WHERE YEAR(rent_finish) = "
              "YEAR(DATE_ADD(CURRENT_DATE, INTERVAL 1 MONTH)) AND MONTH(rent_finish) = MONTH(DATE_ADD(CURRENT_DATE, INTERVAL 1 MONTH))"),
